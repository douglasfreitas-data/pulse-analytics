/**
* ============================================
* PULSE ANALYTICS v17 - 400Hz HIGH SNR
* ============================================
* 
* BASEADO NA v17_1khz
* 
* MUDANÇAS v17_0.4khz:
* - 400 Hz: Taxa otimizada para SNR
* - pulseWidth = 411µs: Máxima integração de luz (18-bit ADC)
* - Mesmo PSRAM allocation
* 
* OBJETIVO:
* Maximizar SNR (Signal-to-Noise Ratio) para 
* detecção de picos mais limpa e confiável.
* 
...
// ============================================
const int BUFFER_SIZE = 30000;  // 75 segundos @ 400Hz
uint16_t* irBuffer = nullptr;   // Será alocado na PSRAM
uint16_t* redBuffer = nullptr;  // Será alocado na PSRAM
int bufferIndex = 0;

// Timing - 400 Hz HIGH SNR
const int SAMPLE_RATE_HZ = 400;
const unsigned long SAMPLE_INTERVAL_US = 2500;  // 2500µs = 2.5ms = 400Hz
...
void uploadRawData() {
  ...
  // JSON
  sendChunk(client, "{");
  sendChunk(client, "\"device_id\": \"ESP32-S3-v17-400Hz-HighSNR\",");
...
void handleCollection() {
  ...
  if (elapsedMS - lastDisplayUpdate >= 1000) {
    ...
    printCentered("v17 400Hz SNR", 54);
    display.display();
...
void showWaitingScreen() {
  ...
  printCentered("v17 400Hz SNR", 12);
...
void setup() {
  ...
  Serial.println("\n============================================");
  Serial.println("   PULSE ANALYTICS v17 - 400Hz HIGH SNR");
  Serial.println("   Buffers na PSRAM, pulseWidth=411us");
  Serial.println("   Maximo SNR (Signal-to-Noise Ratio)");
  Serial.println("============================================");
  ...
  // Config para 400 Hz HIGH SNR
  // pulseWidth=411 dá máxima integração de luz = melhor SNR
  byte ledBrightness = 0x90;  // Mantém o mesmo
  byte sampleAverage = 1;     // Sem averaging
  byte ledMode = 2;           // Red + IR
  int sampleRate = 400;       // 400 Hz
  int pulseWidth = 411;       // 411µs (máximo) - 18 bits ADC, melhor SNR
  ...
  Serial.println("Sensor configurado para 400Hz HIGH SNR:");
  Serial.print("  LED Mode: "); Serial.println(ledMode);
  Serial.print("  Sample Rate: "); Serial.println(sampleRate);
  Serial.print("  Pulse Width: "); Serial.print(pulseWidth); Serial.println("µs (18-bit ADC, MAX SNR)");
...
void loop() {
  ...
  if (digitalRead(0) == LOW) {
    ...
    if (digitalRead(0) == LOW) {
      ...
      if (currentState == WAITING_BUTTON) {
        Serial.println("Botao BOOT: Iniciando coleta 400Hz!");
        display.clearDisplay();
