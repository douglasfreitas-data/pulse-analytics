...
void setup() {
  ...
  // v12: Configuração de Equilíbrio para 800Hz
  byte ledBrightness = 0x3F; // Ganho médio (64) para evitar o "serrilhado"
  byte sampleAverage = 1;    // Sem média para não perder o Nó Dicrótico
  byte ledMode = 2;          // Red + IR
  int sampleRate = 800;      // Frequência alvo
  int pulseWidth = 118;      // Menor tempo de exposição = sinal mais limpo em alta frequência
  int adcRange = 4096;       // Resolução total
  
  particleSensor.setup(ledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange);
  
  // Amplitude dos LEDs - configuração padrão
