...
// Buffer 24000 @ 200Hz = 120 seconds
const unsigned long SAMPLE_DURATION = 120000;
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println(" Falha! Verifique o sinal.");
  }
}
...
void uploadSession(HRVSession *s) {
  ...
  display.println("Subindo Dados...");
  display.setCursor(0, 30);
  display.println("Supabase Cloud");
  display.display();
  ...
  // Start JSON
  sendChunk(client, "{");
  sendChunk(client, "\"device_id\": \"ESP32-S3\",");
  ...
  auto streamArray = [&](String key, uint16_t* data, int count) {
    ...
    for(int i=0; i < count; i++) {
        ...
        // Flush buffer every 200 items to keep memory usage low and reduce chunk overhead
  ...
  client.stop();
  currentSessionTags = ""; // Reset tags after successful session upload
}
...
// BLE removed for stability

void processCommand(String input) {
  // Support for multiple commands in one string (e.g. from a macro)
  int start = 0;
  int end = input.indexOf('\n');
  
  while (end != -1 || start < input.length()) {
    String cmd = (end == -1) ? input.substring(start) : input.substring(start, end);
    cmd.trim();
...
void processSingleCommand(String input) {
  String response = "";
  
  if (input.equalsIgnoreCase("l")) {
    printLog();
    response = "Enviando log via Serial...";
    ...
    if (newName.length() > 0) {
      ...
      saveUserConfig();
      response = "Usuario: " + currentUserName;
    }
  } else if (input.startsWith("TAG:") || input.startsWith("tag:")) {
    String newTags = input.substring(4);
    newTags.trim();
    currentSessionTags = newTags;
    response = "Tag: " + currentSessionTags;
  } else if (input.startsWith("AGE:") || input.startsWith("age:")) {
    String newAgeStr = input.substring(4);
    newAgeStr.trim();
    int val = newAgeStr.toInt();
    if (val >= 15 && val <= 80) {
      currentSessionAge = val;
      response = "Idade: " + String(currentSessionAge);
    } else {
      response = "Err: Idade (15-80)";
    }
  } else if (input.startsWith("SEX:") || input.startsWith("sex:")) {
    String newSex = input.substring(4);
    newSex.trim();
    newSex.toLowerCase();
    if (newSex == "m" || newSex == "f") {
      currentSessionGender = newSex;
      response = "Sexo: " + currentSessionGender;
    } else {
      response = "Err: Sexo (m/f)";
    }
  }
...
void handleMeasurement() {
  // 1. Controle de Tempo Rígido (200Hz = 5000us)
  static unsigned long lastSampleMicros = 0;
  if (micros() - lastSampleMicros < 5000) return; 
  ...
  // 3. Armazenamento Silencioso (Data Science)
  ...
  // Display (Apenas a cada 20 amostras = 10Hz para não travar o loop)
  ...
  if (displayCount++ % 20 == 0) {
    ...
    display.setCursor(0, 50);
    long remaining = (long)SAMPLE_DURATION - (long)elapsedMS;
    if (remaining < 0) remaining = 0;
    display.print(remaining / 1000);
    display.print("s p/ concluir");
