...
// RAM Req: 40k * 2ch * 2 bytes = 160KB. (Libera ~80KB para SSL)
const int BUFFER_SIZE = 40000;  // Session 18 usou esse valor
...
const unsigned long SAMPLE_INTERVAL_US = 1250;  // 1000000 / 800
const unsigned long SAMPLE_DURATION_MS = 50000; // 50 segundos (limite RAM)
...
void uploadRawData() {
  ...
  client.setInsecure();
  client.setTimeout(120000);  // 120 segundos para upload de 160KB
  ...
  sendChunk(client, "{");
  sendChunk(client, "\"device_id\": \"ESP32-S3-v13-Session18-Replica\",");
...
/**
 * ============================================
 * PULSE ANALYTICS v13.0 - SESSION 18 REPLICA
 * ============================================
 * 
 * BASEADO EM: v10.0 Stable (código limpo que funcionava)
 * MODIFICADO PARA: 800Hz / 50s (config da Session 18)
 * 
 * Configuração:
 * - LED Mode: 2 (Red + IR)
 * - Sample Rate: 800Hz
 * - Pulse Width: 215μs
 * - IR Power: 0x7F (127) - MÁXIMO necessário para 800Hz
 * - Red Power: 0x7F (127)
 * 
 * Estratégia:
 * - Buffer local para 50 segundos (RAM)
...
void setup() {
  ...
  Serial.println("\n============================================");
  Serial.println("   PULSE ANALYTICS v13.0 - SESSION 18 REPLICA");
  Serial.println("   Sensor: MAX30102 @ 800Hz / 50s");
  Serial.println("============================================");
  ...
  display.setCursor(0, 10);
  display.println("PulseAnalytics v13");
  display.setCursor(0, 25);
  display.println("SESSION 18 REPLICA");
  display.setCursor(0, 40);
  ...
  int sampleRate = 800;       // 800Hz - Session 18
  int pulseWidth = 215;       // 215us - necessário para 800Hz
  int adcRange = 16384;       // 16-bit range
  
  particleSensor.setup(ledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange);
  
  // Ajustar amplitude dos LEDs - MÁXIMO para 800Hz
  // IMPORTANTE: Em 800Hz (pulseWidth=215us), coleta METADE da luz que 400Hz (pulseWidth=411us)
  // Por isso PRECISAMOS de gain máximo (0x7F) para ter sinal forte
  particleSensor.setPulseAmplitudeRed(0x7F);   // Máximo
  particleSensor.setPulseAmplitudeIR(0x7F);    // MÁXIMO - Session 18 usou isso
