...
const int BUFFER_SIZE = 24000;  // 60 segundos de segurança inicial
...
const unsigned long SAMPLE_INTERVAL_US = 2500;  // 1000000 / 400 = 2500μs
const unsigned long SAMPLE_DURATION_MS = 60000; // 60 segundos (começar conservador)
...
void uploadRawData() {
  ...
  sendChunk(client, "{");
  sendChunk(client, "\"device_id\": \"ESP32-S3-v8-MAX30102\",");
 ...
 * - LED Mode: 2 (Red + IR)
 * - Sample Rate: 400Hz
 * - Pulse Width: 411μs (18-bit range)
 * - IR Power: 0x70 (112) - Otimizado para Indicador
 * - Red Power: 0x7F (127)
 * 
 * Estratégia:
 * - Buffer local para 60 segundos (RAM)
...
void setup() {
  ...
  Serial.println("\n============================================");
  Serial.println("   PULSE ANALYTICS v10.0 - STABLE INDICATOR");
  Serial.println("   Sensor: MAX30102 (Red + IR)");
  Serial.println("============================================");
  ...
  display.setCursor(0, 10);
  display.println("PulseAnalytics v10");
  display.setCursor(0, 25);
  display.println("STABLE INDICATOR");
  display.setCursor(0, 40);
  ...
  byte ledBrightness = 0x3F;  // 63 - Brilho ALTO para sinal forte
  byte sampleAverage = 1;     // Sem média - mantém 400Hz
  byte ledMode = 2;           // Red + IR (MAX30102)
  int sampleRate = 400;       // 400Hz
  int pulseWidth = 411;       // 18-bit ADC - MELHOR RESOLUÇÃO!
  int adcRange = 16384;       // 16-bit range - MAIS SENSIBILIDADE!
  
  particleSensor.setup(ledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange);
  
  // Ajustar amplitude dos LEDs - MÁXIMO para sinal forte
  particleSensor.setPulseAmplitudeRed(0x7F);   // Alto (127) - Manteve igual
  particleSensor.setPulseAmplitudeIR(0x70);    // Reduzido (112) - Margem de seguranca para nao estourar 65k
